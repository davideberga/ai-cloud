...
def reduce_function_global(key_values):
    ...
    try:
        ...
        # Debug: Print key and number of values
        print(f"Processing center: {center}, values count: {len(values)}")
        
        for i, value in enumerate(values):
            try:
                # Debug: Print value details
                print(f"Processing value {i}: type={value.type}, center={value.center}")
                
                if center != value.center:
                    error_msg = f"Center mismatch: {center} != {value.center}"
                    print(error_msg)
                ...
                if value.type == STAR_GRAPH:
                    ...
                    print(f"Found star graph info, triplets: {len(value.triplet_graphs)}")
                    ...
                    deg_cnt += 1
                    print(f"Added neighbor: {value.target} with weight {value.weight}")
                    
            except Exception as value_error:
                error_msg = f"Error processing value {i} for center {center}: {value_error}"
                print(error_msg)
                raise
        
        # Check if we have partition information
        if count_info_partitions == 0:
            print(f"No partition info for center {center}, skipping")
            return []
        
        if count_info_partitions != 1:
            error_msg = f"Expected 1 partition info, got {count_info_partitions} for center {center}"
            print(error_msg)
            raise AssertionError(error_msg)
        ...
        print(f"Created star graph for center {center} with {len(neighbors)} neighbors and {len(triples)} triples")
        return [star]
        
    except Exception as e:
        error_msg = f"[STAR_GRAPH_GEN] Reduce Error for key {key if 'key' in locals() else 'unknown'}: {traceback.format_exc()}"
        print(error_msg)  # Use print instead of logger in worker
        raise


# Alternative reduce function with degree checking (if needed)
def reduce_function_with_degree_check(key_values, degree_dict):
    """
    Reduce function with degree checking - requires passing degree_dict separately
    """
    try:
        key, values = key_values
        center = key
        ...
        # Get degree from dictionary
        deg_center = degree_dict.get(center, 0)
...
class LoopGenStarGraphWithPrePartitions:
    ...
    def run(self, args: List[str]) -> int:
        ...
        try:
            ...
            # Apply reduce function with error handling
            try:
                ...
                # Count for logging with better error handling
                logger.info("Computing star graph count...")
                star_count = star_graphs_rdd.count()
                logger.info(f"Generated {star_count} star graphs")
                
            except Exception as e:
                logger.error(f"Error in reduce phase: {traceback.format_exc()}")
                # Try to collect first few elements to see the actual error
                try:
                    sample_results = star_graphs_rdd.take(5)
                    logger.info(f"Successfully generated {len(sample_results)} sample star graphs")
                    star_count = len(sample_results)  # Use sample count if full count fails
                except Exception as sample_error:
                    logger.error(f"Error in sample collection: {traceback.format_exc()}")
                    raise
