...
from attractor.LoopGenStarGraphWithPrePartitions import (
    LoopGenStarGraphWithPrePartitions,
)
...
from attractor.LoopDynamicInteractionsFasterNoCache import (
    LoopDynamicInteractionsFasterNoCache,
)
import warnings
...
class MasterMR:
    ...
    def compute_load_balanced_greedy(
        self,
        workload_estimate_out,
        graphname,
        hdfs,
        localsystem,
        conf,
        ...
        MyUtil.merge_files(
            workload_estimate_out, out_est_load_file_merged, hdfs, localsystem, conf
        )
                ...
                for i in range(K, N):
                    ...
                    if self.DEBUG:
                        if workload_heap:
                            ...
                            assert (
                                next_bucket.total_workload
                                >= top_smallest.total_workload
                            )
    ...
    def master(
        ...
        df_graph_jaccard = graph_with_jaccard.get_graph_jaccard_dataframe(self.spark).rdd
        df_graph_degree = graph_with_jaccard.get_degree_dict()

        rdd_graph_degree_broadcasted = self.sc.broadcast(df_graph_degree)
        ...
        df_partitioned = partition_computer.compute(
            df_graph_jaccard, no_partition_dynamic_interaction
        )
        ...
        while flag:
            ...
            rdd_star_graph = star_graph.compute(
            ...
            rdd_dynamic_interactions = dynamic_interactions.compute(
                rdd_star_graph,
                no_partition_dynamic_interaction,
                lambda_val,
            ...
            if self.REDUCED_EDGE:
                current_left_edges_file = (
                    f"converged_edges_{a[-1]}_{self.cache_type_message}_{cnt_round + 1}"
                )
                reduced_local_edge_file = (
                    f"local_edge_file_{a[-1]}_reduced_local_{cnt_round + 1}.txt"
                )
                reduced_edges_folder = f"{out_update_edge}/reduced_edges"

                print(f"Reducing the number of edges @Loops: {cnt_round + 1}")
                info = self.reduce_edges(
                    N,
                    curr_edge_folder,
                    local_filesystem,
                    None,
                ...
                self.log_job.write(
                    f"Running time of {cnt_round + 1} iteration: {length_each_iteration}\n"
                )
                self.log_job.flush()
                ...
                if non_converged_edges <= threshold_used_edges:
                    ...
                    self.log_job.write(
                        f"Dynamic Interaction Time of MR version: {dynamic_time_mr / 1000.0}\n"
                    )
                    self.log_job.flush()
                    ...
                    if using_sliding_window:
                        binary_sliding_windows_folder_local = (
                            f"sliding_merged_{a[-1]}MR"
                        )
                        MyUtil.copy_folder(
                            current_sliding_window_folder,
                            binary_sliding_windows_folder_local,
                        )
                        merged_sliding_windows_file_normal = (
                            f"sliding_merged_{a[-1]}_normal"
                        )
                    ...
                    merged_all_converged_edges = (
                        f"mergedAllConvergedEdges_{a[-1]}.edges"
                    )
                    ...
                    self.time_running_on_single_machine += (
            ...
            if flag and MyUtil.path_exists(f"{out_update_edge}/flag"):
                ...
                self.log_job.write(
                    f"Init Interaction Time (duplicated info){initialization_time / 1000.0}\n"
                )
                self.log_job.write(
                    f"Dynamic Interaction Time (single + MR) (seconds) {dynamic_time / 1000.0}\n"
                )
                self.log_job.write(
                    f"Running time generating star graphs: {self.time_generating_star_graph}\n"
                )
                self.log_job.write(
                    f"Running time computing dynamic interactions: {self.time_computing_dynamic_interactions}\n"
                )
                self.log_job.write(
                    f"Running time updating edges: {self.time_updating_edges}\n"
                )
                self.log_job.write(
                    f"Running time on single machine: {self.time_running_on_single_machine}\n"
                )
                ...
                self.log_job.write(
                    f"Copying Final Edge file: {copying_time / 1000.0}\n"
                )
                ...
                self.log_job.write(
                    f"Breadth First Search Time: {breath_first_search_time / 1000.0}\n"
                )
                ...
                self.log_job.write(
                    f"Total Running Time (seconds): {three_phases / 1000.0}\n"
                )
                self.log_job.close()
                break
...
def main():
    ...
    if len(args) < 16:
        raise AssertionError(
            f"Number of arguments must be >=16 while current input has: {len(args)} arguments."
        )
