...
class MRPreComputePartition:
    ...
    def mapReduce(rdd_edge_with_jaccard: DataFrame, n_partitions: int) -> int:
        ...
        reduce_output = combined_output.map(MRPreComputePartition.reduce_function)  # Dataframe with ("S", vertex_id, len(triples), triples) where vertex_id is the center of the star graph in the next steps: END")
